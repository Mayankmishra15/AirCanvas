...
mphand=mp.solutions.hands #From solution we select hands 
hand=mphand.Hands()
...
while True :
    ...
    imgrgb=cv2.cvtColor(img,cv2.COLOR_BGR2RGB)#image is converted to rgb
    ...
    if result.multi_hand_landmarks:
        for handlm in  result.multi_hand_landmarks:
            for id,lm in enumerate(handlm.landmark):
                # print(id,lm)
                h,w,c=img.shape  #height , width & channel return 
...
paintWindow = cv2.rectangle(paintWindow, (40, 1), (140, 65), (0, 0, 0), 2) #black
...
cv2.waitKey(1)
#part 3 draw color panels on webcam window
import cv2
import mediapipe as mp
import numpy as np 
